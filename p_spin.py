...
class PatternSpin(Pattern):
    def __init__(self, soul, board, bullets, center, set_battle_time, interval = 40, nvel = 6, nacc = 0.05, rvel = 7, racc = 0.05, reverseDelay = 500):
    ...
    def barrier(self, barrier):
        ...
        for i in barrier:
            ...
            bullet = BulletStatic(x, y, 180, sprite, breakable, 17)
    ...
    def update(self):
        ...
        for i in self.children:
            isOff = not((i.y < (self.board.y + self.board.size/2)) and (i.y > (self.board.y - self.board.size/2)))

            if self.prevAreOff[i] and not isOff:
                    i.morph_to(break_bullet_image, 0.2)
            if not self.prevAreOff[i] and isOff:
                    i.morph_to(alpha_image, 0.6)
