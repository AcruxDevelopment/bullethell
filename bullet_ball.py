...
def loadAssets():
	...
	if not(spade_image is None) and not(spade_spawn_image is None):
		return
	try:
		spade_spawn_image = Textures.scaleToFit(Textures.load("alpha.webp"), 60, 60)
		spade_image = Textures.scaleToFit(Textures.load("ball.webp"), 60, 60)
